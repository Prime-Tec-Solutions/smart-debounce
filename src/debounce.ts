...
export function debounce<F extends AnyFunction>(
  ...
  async function tryRun() {
    try {
      ...
      // Clear state before running to avoid reentrancy issues
      clearTimers();
      cleanupKeyListener();
      hasPending = false;
      lastArgs = null;
  ...
  function schedule() {
    if (!hasPending) {
      ...
      if (maxWait != null && Number.isFinite(maxWait) && maxWait > 0) {
        // Fire at most after maxWait from the start of the pending window
        maxWaitTimer = setTimeout(() => {
          // Force execution attempt (still respects condition)
          if (hasPending) {
            if (delayTimer) {
              clearTimeout(delayTimer);
              delayTimer = null;
  ...
  function flush(): any {
    ...
    // If we can run immediately and return synchronously (sync fn and sync-true condition), do so
    ...
    if (canRunSyncNow) {
      ...
      lastThis = null;
      try {
        // Run synchronously and return its value
        return (fn as any).apply(currentThis, currentArgs);
      } catch (err) {
        // For sync functions, propagate the error
        throw err;
      }
    }

    // Otherwise, trigger soon and return promise only for async functions
    if (delayTimer) {
      clearTimeout(delayTimer);
      delayTimer = null;
    }
  ...
  const debounced = function (this: any, ...args: any[]) {
    ...
    if (isDeclaredAsyncFunction) {
      // For async functions, return a promise representing this pending call
      return ensurePendingPromise();
    }
    // For sync functions, return void
    return undefined;
